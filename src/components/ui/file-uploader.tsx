...
interface FileUploaderProps extends React.HTMLAttributes<HTMLDivElement> {
   ...
   */
  onValueChange?: React.Dispatch<React.SetStateAction<FileUpload[]>>;
  ...
  const onDrop = React.useCallback(
    (acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
      if (!multiple && maxFiles === 1 && acceptedFiles.length > 1) {
        toast.error(
          "Não é possível fazer upload de mais de um arquivo por vez",
        );
        return;
      }
      ...
      if ((files?.length ?? 0) + acceptedFiles.length > maxFiles) {
        toast.error(
          `Não é possível fazer upload de mais de ${maxFiles} arquivos`,
        );
        return;
      }
      ...
      if (
        ...
        toast.promise(onUpload(updatedFiles), {
          ...
          success: () => {
            setFiles([]);
            return `${target} carregado`;
          },
          error: `Falha ao carregar ${target}`,
        });
      }
    },
        ...
        {({ getRootProps, getInputProps, isDragActive }) => (
          <>
            <div
              {...getRootProps()}
              className={cn(
                "group relative grid h-40 w-full cursor-pointer place-items-center rounded-lg border-2 border-dashed border-muted-foreground/25 px-5 py-2.5 text-center transition hover:bg-muted/40",
            ...
            <p
              className={cn(
                "mx-6 text-center text-xs text-muted-foreground",
...
interface FileCardDoneProps {
  file: File;
  onRemove: () => void;
}

export function FileCardDone({ file, onRemove }: FileCardDoneProps) {
  return (
